...
if (
    ...
    if (isset($_POST['u_id_formation'])){
        ...
        if (!empty($_POST['u_nformation'])) {
            $stmt = $bdd->prepare("UPDATE formation SET nom_formation=:nom_formation WHERE id_formation=" . $_POST['u_id_formation']);
            $stmt->execute(array('nom_formation' => $_POST['u_nformation']));
            $_SESSION['error_msg'] = '';
        }
        // 'u_lieu'
        if (!empty($_POST['u_lieu'])) {
            $stmt = $bdd->prepare("UPDATE formation SET lieu=:lieu WHERE id_formation=" . $_POST['u_id_formation']);
            $stmt->execute(array('lieu' => $_POST['u_lieu']));
            $_SESSION['error_msg'] = '';
        }
        // 'u_nb_eleve_max'
        if (!empty($_POST['u_nb_eleve_max'])) {
            $stmt = $bdd->prepare("UPDATE formation SET nb_eleve_max=:nb_eleve_max WHERE id_formation=" . $_POST['u_id_formation']);
            $stmt->execute(array('nb_eleve_max' => $_POST['u_nb_eleve_max']));
            $_SESSION['error_msg'] = '';
        }
        // 'u_lien_formation_slack'
        if (!empty($_POST['u_lien_formation_slack'])) {
            $stmt = $bdd->prepare("UPDATE formation SET lien_formation_slack=:lien_formation_slack WHERE id_formation=" . $_POST['u_id_formation']);
            $stmt->execute(array('lien_formation_slack' => $_POST['u_lien_formation_slack']));
            $_SESSION['error_msg'] = '';
        }
        // 'u_date_de__debut'
        if (!empty($_POST['u_date_de__debut'])) {
            $stmt = $bdd->prepare("UPDATE formation SET date_de__debut=:date_de__debut WHERE id_formation=" . $_POST['u_id_formation']);
            $stmt->execute(array('date_de__debut' => $_POST['u_date_de__debut']));
            $_SESSION['error_msg'] = '';
        }
        // 'u_date_de_fin'
        if (!empty($_POST['u_date_de_fin'])) {
            $stmt = $bdd->prepare("UPDATE formation SET date_de_fin=:date_de_fin WHERE id_formation=" . $_POST['u_id_formation']);
            $stmt->execute(array('date_de_fin' => $_POST['u_date_de_fin']));
            $_SESSION['error_msg'] = '';
        }
        // 'u_id_user'
        if (!empty($_POST['u_id_user'])) {
            $stmt = $bdd->prepare("UPDATE formation SET id_user=:id_user WHERE id_formation=" . $_POST['u_id_formation']);
            $stmt->execute(array('id_user' => $_POST['u_id_user']));
            $_SESSION['error_msg'] = '';
        }
        // 'u_id_categorie'
        if (!empty($_POST['u_id_categorie'])) {
            $stmt = $bdd->prepare("UPDATE formation SET id_categorie=:id_categorie WHERE id_formation=" . $_POST['u_id_formation']);
